...
public class PaymentManagementController {
    ...
    @FXML private TableColumn<Payment, String> colEnroll;   // giữ fx:id cũ nhưng hiển thị ORDER
    ...
    // FORM PAYMENT (Order)
    @FXML private ComboBox<Order> cbEnrollment;  // giữ fx:id cũ cho khỏi sửa FXML, nhưng type là Order
    ...
    public void initialize() {
        ...
        tblPayments.getSelectionModel().selectedItemProperty().addListener((obs, old, cur) -> {
            if (cur != null) loadRefunds(cur.getId());
            else refunds.clear();
        });
    ...
    private void loadOrders() {
        try (Session s = HibernateUtil.getSessionFactory().openSession()) {
            var list = s.createQuery(
                    "select o from Order o join fetch o.student st " +
                            "where o.status in ('PENDING','PENDING_PAYMENT') " +
                            "order by o.id desc",
                    Order.class
            ).list();
            cbEnrollment.setItems(FXCollections.observableArrayList(list));
            if (!list.isEmpty()) cbEnrollment.setValue(list.get(0));
        }
    }

    @FXML
    private void handleFilter() {
        payments.setAll(paymentRepo.filterFetchUI(cbFilterStatus.getValue(), cbFilterMethod.getValue(), txtSearch.getText()));
        info("ℹ Đã lọc");
    }
    ...
    private void handleCreatePayment() {
        ...
        if (o == null) { err("Chưa chọn Order"); return; }

        BigDecimal amount = parseMoneySmart(txtAmount.getText());
        if (amount == null || amount.signum() <= 0) { err("Số tiền không hợp lệ"); return; }

        PaymentMethod method = cbMethod.getValue();
        PaymentStatus status = cbPayStatus.getValue();
        if (method == null || status == null) { err("Chưa chọn method/status"); return; }
        ...
        try (Session s = HibernateUtil.getSessionFactory().openSession()) {
            ...
            if (managedO == null) throw new IllegalStateException("Order không tồn tại");
            ...
            if (status == PaymentStatus.PAID) p.setPaidAt(LocalDateTime.now());
            s.persist(p);
    ...
    private void handleMarkPaid() {
        ...
        if (p == null) { err("Chọn 1 Payment"); return; }
        ...
        try (Session s = HibernateUtil.getSessionFactory().openSession()) {
            ...
            if (mp == null) throw new IllegalStateException("Payment không tồn tại");
            ...
            if (o == null) throw new IllegalStateException("Payment chưa gắn Order (order_id null)");
    ...
    private void handleRequestRefund() {
        Payment p = tblPayments.getSelectionModel().getSelectedItem();
        if (p == null) { err("Chọn Payment để hoàn"); return; }

        BigDecimal amount = parseMoneySmart(txtRefundAmount.getText());
        if (amount == null) amount = nz(p.getAmount());
        if (amount.signum() <= 0) { err("RefundAmount không hợp lệ"); return; }
    ...
    @FXML private void handleApproveRefund() { updateRefundStatus(RefundStatus.APPROVED, "✅ Đã Approve"); }
    @FXML private void handleRejectRefund()  { updateRefundStatus(RefundStatus.REJECTED,  "✅ Đã Reject"); }

    private void updateRefundStatus(RefundStatus st, String okMsg) {
        Refund r = tblRefunds.getSelectionModel().getSelectedItem();
        if (r == null) { err("Chọn 1 Refund"); return; }
        ...
        try (Session s = HibernateUtil.getSessionFactory().openSession()) {
            ...
            if (mr == null) throw new IllegalStateException("Refund không tồn tại");
    ...
    private void handleProcessRefund() {
        ...
        try (Session s = HibernateUtil.getSessionFactory().openSession()) {
            ...
            if (selected != null) loadRefunds(selected.getId());
            loadOrders();
            loadAll();
    ...
    private void updateOrderAndEnrollmentByNetPaid(Session s, Order o) {
        ...
        BigDecimal paid = (BigDecimal) s.createQuery(
        ...
        BigDecimal refunded = (BigDecimal) s.createQuery(
                "select coalesce(sum(r.amount),0) from Refund r " +
                        "join r.payment p " +
                        "where p.order.id = :oid and r.status = :rst"
        ).setParameter("oid", o.getId())
         .setParameter("rst", RefundStatus.PROCESSED)
         .uniqueResult();

        BigDecimal netPaid = nz(paid).subtract(nz(refunded));
        if (netPaid.signum() < 0) netPaid = BigDecimal.ZERO;

        BigDecimal due = total.subtract(netPaid);
        if (due.signum() <= 0) o.setStatus("PAID");
        else o.setStatus("PENDING_PAYMENT");

        s.merge(o);

        // ✅ đồng bộ paidAmount vào enrollment để các màn Student/Payment hiển thị “còn nợ” đúng
        if (e != null) {
    ...
    private void ok(String msg) { lblMsg.setStyle("-fx-text-fill:#16a34a; -fx-font-weight:bold;"); lblMsg.setText(msg); }
    private void err(String msg) { lblMsg.setStyle("-fx-text-fill:#ef4444; -fx-font-weight:bold;"); lblMsg.setText("❌ " + msg); }
    private void info(String msg){ lblMsg.setStyle("-fx-text-fill:#2563eb; -fx-font-weight:bold;"); lblMsg.setText(msg); }
}
